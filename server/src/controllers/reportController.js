...
const RETURN_STEP_SQL = `
    CASE
      ...
      WHEN dt.NgayBanGiao IS NOT NULL AND dt.MaNV_XuLy IS NULL AND dt.TrangThai NOT IN (N'Hoàn thành', N'Từ chối', N'Đã hủy') THEN N'Sót từ ca trước — chờ thu ngân ca mới tiếp nhận'
      WHEN dt.TrangThai=N'Đã duyệt' AND dt.NgayBanGiao IS NOT NULL THEN N'Chờ thu ngân ca đang mở xác nhận hoàn/đổi'
