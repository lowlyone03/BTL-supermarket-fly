...
const mapHttpError = (status, payload, raw = '') => {
    ...
    if (status === 401 || code === 'invalid_api_key' || code === 'authentication_error') {
        const error = new Error('Khóa Genspark không hợp lệ hoặc đã thu hồi. Kiểm tra ASSISTANT_API_KEY rồi chạy lại npm start.');
        error.status = 401;
        error.code = 'ASSISTANT_AUTH';
        return error;
    }
    if (status === 402 || code === 'insufficient_funds') {
        const error = new Error('Hết hạn mức Genspark. Nạp thêm tín dụng hoặc nâng gói, rồi hỏi lại — hoặc mở đúng màn hình dashboard.');
        error.status = 402;
        error.code = 'ASSISTANT_QUOTA';
        return error;
    }
    if (status === 429 || code === 'rate_limit_exceeded') {
        const error = new Error('Genspark đang giới hạn tốc độ. Đợi một lát rồi hỏi lại, hoặc mở đúng màn hình dashboard.');
        error.status = 429;
        error.code = 'ASSISTANT_RATE';
        return error;
    }
    if (status === 403 || code === 'insufficient_scope') {
        const error = new Error('Khóa Genspark thiếu quyền gọi chat. Kiểm tra API key trên tài khoản Genspark.');
        error.status = 403;
        error.code = 'ASSISTANT_SCOPE';
        return error;
    }
    if (status === 502 || looksLikeCloudflare(payload, raw)) {
        const error = new Error('Genspark tạm gián đoạn (lỗi cổng máy chủ). Đợi ít phút rồi hỏi lại, hoặc mở đúng màn hình dashboard.');
        error.status = 502;
...
const complete = async ({ system, user, maxTokens = 700 } = {}) => {
    ...
    const body = JSON.stringify({
        ...
        stream: false,
        temperature: 0.2,
        max_tokens: Math.min(Math.max(Number(maxTokens) || 700, 256), 2048),
        messages: [
