    ...
    'products.html': `
      <section class="admin-module">
        <header class="module-heading">
          <div><p class="module-kicker">DỮ LIỆU DÙNG CHUNG / HÀNG HÓA</p><h1>Sản phẩm &amp; giá</h1><p>Chốt danh mục, giá bán, thuế suất VAT (0 / 5 / 8 / 10) và mức tồn tối thiểu. Sản phẩm chưa chọn thuế (NULL) sẽ bị POS chặn bán.</p></div>
        ...
        <article class="surface-card data-surface">
          <div class="table-toolbar"><label class="filter-search"><svg aria-hidden="true"><use href="#i-search"/></svg><input id="productSearch" placeholder="Tìm mã, tên sản phẩm hoặc mã vạch..."></label><div class="filter-actions"><select id="productCategory"><option value="">Tất cả danh mục</option></select><select id="productStatus"><option value="">Tất cả trạng thái</option><option>Đang bán</option><option>Ngừng bán</option></select><select id="productTaxFilter" aria-label="Lọc thuế suất"><option value="">Tất cả thuế</option><option value="missing">Chưa chọn thuế</option><option value="0">0%</option><option value="5">5%</option><option value="8">8%</option><option value="10">10%</option></select><button class="icon-button" onclick="loadProducts()" aria-label="Tải lại"><svg><use href="#i-refresh"/></svg></button></div></div>
          <div class="product-tax-alert" id="productTaxAlert" hidden>Có sản phẩm chưa chọn thuế suất. Gán 0 / 5 / 8 / 10 — nếu để trống (NULL) thì POS chặn bán.</div>
          <div class="table-container"><table><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Giá nhập / bán</th><th>Thuế suất</th><th>Tồn tối thiểu</th><th>Tồn hiện tại</th><th>Trạng thái</th><th class="align-right">Thao tác</th></tr></thead><tbody id="productTableBody"></tbody></table></div>
        </article>
      </section>
      <div class="modal-backdrop" id="productModal" style="display:none"><div class="modal modal-wide product-editor-modal"><div class="modal-header"><div><p class="module-kicker">HỒ SƠ SẢN PHẨM</p><h3 id="productModalTitle">Thêm sản phẩm</h3><p class="modal-description" id="productModeNote">Khai báo mã, danh mục, giá và mức tồn tối thiểu trước khi nhập hàng.</p></div><button class="close-btn" onclick="closeProductModal()" aria-label="Đóng">×</button></div><div class="modal-body"><form id="productForm"><div class="form-grid">
        ...
        <div class="form-group form-span-2"><label>Tên sản phẩm *</label><input id="productName" maxlength="150" required placeholder="Nhập tên đầy đủ trên bao bì"><small class="field-error" id="productNameError"></small></div>
        <div class="form-group"><label>Đơn vị tính *</label><input id="productUnit" list="productUnitSuggestions" maxlength="30" required placeholder="Chọn hoặc nhập đơn vị"><datalist id="productUnitSuggestions"><option value="Gói"><option value="Hộp"><option value="Chai"><option value="Lon"><option value="Thùng"><option value="Cái"><option value="Kg"></datalist><small class="field-error" id="productUnitError"></small></div><div class="form-group"><label>Mã vạch</label><div class="input-with-action"><input id="productBarcode" inputmode="numeric" maxlength="30" placeholder="Quét/nhập mã trên bao bì"><button type="button" class="field-action" onclick="suggestProductBarcode()">Gợi ý</button></div><small class="field-help">Không bắt buộc; mã vạch phải duy nhất nếu có.</small><small class="field-error" id="productBarcodeError"></small></div>
        <div class="form-group"><label>Giá nhập *</label><input id="productCost" type="number" min="0" step="100" required placeholder="0"><small class="field-error" id="productCostError"></small></div><div class="form-group"><label>Giá bán *</label><input id="productPrice" type="number" min="0" step="100" required placeholder="0"><small class="field-error" id="productPriceError"></small></div>
        <div class="form-group"><label>Thuế suất VAT *</label><select id="productTaxRate" required><option value="">Chưa chọn</option><option value="0">0%</option><option value="5">5%</option><option value="8">8%</option><option value="10">10%</option></select><small class="field-help">Bắt buộc 0 / 5 / 8 / 10. 0% là mức hợp lệ, không phải «chưa chọn».</small></div>
      ...
      <script src="../admin/products.js?v=pending-search-1"></script>`,
