...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const syncSelectControl = select => {
    ...
    trigger.querySelector('span').textContent = selected?.textContent || t('common.choose');
  ...
  window.FLY_APPEARANCE?.bootFromStorage(user.MaNV);
  ...
  const paintIdentity = () => {
    ...
    const roleText = window.FLY_I18N?.roleLabel(user.TenVaiTro) || (user.TenVaiTro === 'Quản lý' ? t('role.managerStore') : user.TenVaiTro);
  ...
  const paintSearchPlaceholder = () => {
    document.getElementById('globalSearch').placeholder = window.FLY_I18N?.searchPlaceholder(searchRole) || t('top.searchPh');
  };
  ...
  const showInboxToast = (item, extraCount) => {
    const el = document.getElementById('inboxToast');
    if (!el) return;
    document.getElementById('inboxToastTitle').textContent = item.title || t('inbox.newItem');
    const extra = extraCount > 1 ? t('inbox.andMore', { n: extraCount - 1 }) : '';
  ...
  const syncSoundToggle = () => {
    ...
    button.title = muted ? t('top.soundOn') : t('top.soundOff');
  ...
  const refreshCurrentPage = async () => {
    ...
    window.FLY_I18N?.applyDom(contentArea);
    window.FLY_I18N?.applyPhrases(contentArea);
  };
  ...
  const loadOverview = async () => {
    ...
    try {
      ...
      const roleBars = data.roleDistribution.map((role, i) => {
        ...
        return `
        <div class="role-bar-row" style="cursor:pointer" data-open-target="../admin/employees.html" title="${escapeHtml(t('overview.seeStaff', { role: window.FLY_I18N?.roleLabel(role.TenVaiTro) || role.TenVaiTro }))}">
          <span>${escapeHtml(window.FLY_I18N?.roleLabel(role.TenVaiTro) || role.TenVaiTro)}</span>
      ...
      const logRows = data.recentLogs.length ? data.recentLogs.slice(0, 8).map(log => {
        ...
        <li class="activity-item" title="${escapeHtml(absTime + ' · ' + (log.NoiDung || ''))}">
          ...
          <div class="activity-body">
            ...
            <p>${escapeHtml(log.NoiDung || log.NguoiThaoTac || t('overview.systemAct'))}</p>
          </div>
          <div class="activity-meta">
            <time>${timeAgo(log.ThoiGian)}</time>
            <span class="activity-status-badge ${isLogin ? 'login' : 'success'}">${t('common.success')}</span>
          </div>
        </li>`;
      }).join('') : `<li class="empty-row">${t('overview.noLog')}</li>`;
      const priorityTitle = pending.TongChoDuyet > 0
      ...
      contentArea.innerHTML = `
        <section class="overview-page">
          <header class="overview-heading">
            <div><p class="eyebrow">${t('overview.eyebrow')}</p><h1>${t('overview.h1')}</h1><p>${t('overview.lead')}</p></div>
            <div class="date-chip"><svg><use href="#i-calendar"/></svg><span>${t('overview.today', { date: formatToday() })}</span></div>
          </header>

          <article class="priority-banner">
            <div class="priority-copy"><span class="priority-label"><svg><use href="#i-clock"/></svg> ${t('overview.priority')}</span><h2>${priorityTitle}</h2><p>${priorityText}</p><button type="button" data-action="approval">${t('overview.openApprovals')} <svg><use href="#i-chevron"/></svg></button></div>
            <div class="priority-number"><span>${t('overview.ofWhich')}</span><strong>${String(financePending).padStart(2, '0')}</strong><small>${t('overview.financeRelated')}</small></div>
          </article>

          <div class="section-heading"><div><p>${t('overview.needAction')}</p><h2>${t('overview.pendingWork')}</h2></div><button type="button" data-action="approval">${t('overview.seeAll')} <svg><use href="#i-chevron"/></svg></button></div>
          ...
          <div class="stat-grid">
            <article class="stat-card"><div><span>${t('overview.revToday')}</span><i><svg><use href="#i-trend"/></svg></i></div><strong>${formatMoney(data.revenue?.DoanhThuHomNay || 0)}</strong><small>${t('overview.gross', { money: formatMoney(data.revenue?.LaiGopHomNay || 0) })}</small></article>
            <article class="stat-card"><div><span>${t('overview.rev7')}</span><i><svg><use href="#i-report"/></svg></i></div><strong>${formatMoney(data.revenue?.DoanhThu7Ngay || 0)}</strong><small>${t('overview.rev7hint')}</small></article>
            <article class="stat-card"><div><span>${t('overview.openPos')}</span><i><svg><use href="#i-clock"/></svg></i></div><strong>${summary.CaDangMo || 0}</strong><small>${t('overview.openPosHint')}</small></article>
            <article class="stat-card"><div><span>${t('overview.staff')}</span><i><svg><use href="#i-users"/></svg></i></div><strong>${summary.NhanVienDangLam}</strong><small>${t('overview.staffHint')}</small></article>
            <article class="stat-card ${summary.ChuaCoTaiKhoan ? 'attention' : ''}"><div><span>${t('overview.noAccount')}</span><i><svg><use href="#i-user-plus"/></svg></i></div><strong>${summary.ChuaCoTaiKhoan}</strong><small>${t('overview.noAccountHint', { n: summary.ChuaCoTaiKhoan })}</small></article>
            <article class="stat-card ${summary.TaiKhoanBiKhoa ? 'attention' : ''}"><div><span>${t('overview.locked')}</span><i><svg><use href="#i-lock"/></svg></i></div><strong>${summary.TaiKhoanBiKhoa}</strong><small>${t('overview.lockedHint', { n: summary.ThaoTacHomNay })}</small></article>
          </div>

          ${(data.lowStock || []).length > 0 ? `<div class="section-heading"><div><p>${t('overview.stockAlert')}</p><h2>${t('overview.needRestock')}</h2></div><button type="button" data-open-target="../admin/products.html">${t('overview.seeAll')} <svg><use href="#i-chevron"/></svg></button></div>
          <div class="decision-grid">${data.lowStock.slice(0, 6).map(item => `<article class="decision-card red" style="cursor:default"><div class="card-top"><span class="decision-icon"><svg><use href="#i-warning"/></svg></span><strong>${item.SLTon}</strong></div><h3>${escapeHtml(item.TenSP)}</h3><p>${escapeHtml(item.MaSP)} · ${t('overview.minStock', { n: item.TonKhoToiThieu, unit: item.DonViTinh || '' })}</p></article>`).join('')}</div>` : ''}

          <section class="overview-product-showcase">
            <div class="section-heading"><div><p>${t('overview.visual')}</p><h2>${t('overview.watchProducts')}</h2><span>${t('overview.watchHint')}</span></div><button type="button" data-open-target="../admin/products.html">${t('overview.manageProducts')} <svg><use href="#i-chevron"/></svg></button></div>
            <div class="overview-product-grid">${productCards || `<div class="overview-product-empty">${t('overview.noPhoto')}</div>`}</div>
          </section>

          <div class="overview-columns">
            <article class="overview-panel overview-panel-staff">
              <div class="panel-title"><div><p class="module-kicker">${t('overview.staffKicker')}</p><h3>${t('overview.staffTitle')}</h3><p>${t('overview.staffLead')}</p></div><button class="quick-link" data-open-target="../admin/employees.html" aria-label="${t('overview.openStaff')}"><svg><use href="#i-chevron"/></svg></button></div>
              <div class="role-chart">
                <div class="role-chart-summary">
                  <div><span>${t('overview.working')}</span><strong>${summary.NhanVienDangLam}</strong></div>
                  <div><span>${t('overview.roleGroups')}</span><strong>${t('overview.groupsN', { n: data.roleDistribution.length })}</strong></div>
                  <div><span>${t('overview.noTk')}</span><strong class="${summary.ChuaCoTaiKhoan ? 'staff-warn' : ''}">${summary.ChuaCoTaiKhoan}</strong></div>
                </div>
                <div class="role-bars">${roleBars}</div>
              </div>
            </article>
            <article class="overview-panel overview-panel-log">
              <div class="panel-title"><div><p class="module-kicker">${t('overview.logKicker')}</p><h3>${t('overview.logTitle')}</h3></div><button class="quick-link" data-open-target="../admin/audit-log.html" aria-label="${t('overview.openLog')}"><svg><use href="#i-chevron"/></svg></button></div>
              <ul class="activity-list-v2">${logRows}</ul>
              <div class="activity-footer"><a href="#" class="activity-view-all" data-open-target="../admin/audit-log.html">${t('overview.seeLogs')}</a></div>
            </article>
          </div>
        </section>`;
      window.FLY_I18N?.applyPhrases(contentArea);
      ...
      if (loadVersion !== navigationVersion) return;
      contentArea.innerHTML = `<div class="welcome-card"><h2>${t('error.ops')}</h2><p>${escapeHtml(error.message)}</p><button class="btn btn-primary" id="retryOverview">${t('common.retry')}</button></div>`;
  ...
  const resolvePageHtml = async pageName => {
    ...
    if (isAdminPage) {
      ...
      if (!response.ok) throw new Error(t('error.page'));
      return response.text();
    }
    throw new Error(t('error.needRestart'));
  };
  ...
  const openPage = async navItem => {
    ...
    try {
      ...
      if (pageVersion !== navigationVersion) return;
      contentArea.innerHTML = `<div class="welcome-card"><h2>${t('error.page')}</h2><p>${escapeHtml(error.message)}</p><button class="btn btn-primary" id="retryPage">${t('common.retry')}</button></div>`;
  ...
  document.getElementById('pwdForm').addEventListener('submit', async event => {
    ...
    const oldOk = show('oldPwd', Boolean(oldPwd), t('pwd.needOld'));
    const newResult = fields ? fields.validateNewPassword(newPwd) : { ok: Boolean(newPwd.trim()), message: t('pwd.needNew') };
    const newOk = show('newPwd', newResult.ok, newResult.message);
    const confirmOk = show('confirmPwd', newPwd === confirmPwd, t('pwd.mismatch'));
