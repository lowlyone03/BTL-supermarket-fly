(() => {
  ...
  const templates = {
    'warehouse-receiving': '<section class="warehouse-page"><div class="overview-loading">Đang tải hàng chờ nhận...</div></section>',
    'warehouse-receipts': '<section class="warehouse-page"><div class="overview-loading">Đang tải Phiếu nhập kho...</div></section>'
  };
  ...
  const statusClass = status => ({ 'Nháp': 'draft', 'Đã xác nhận': 'ok', 'Nhà cung cấp xác nhận': 'ok', 'Đã gửi Nhà cung cấp': 'processing', 'Đang giao': 'processing', 'Đã đến kho': 'sent', 'Đang kiểm nhận': 'processing', 'Đã kiểm nhận': 'ok', 'Giao một phần': 'returned' }[status] || 'draft');
  ...
  const receivingModal = async (context, shipmentId, onDone) => {
    try {
      ...
      overlay.querySelectorAll('.warehouse-receipt-line[data-product]').forEach(row => {
        const delivered = row.querySelector('.delivered'); const accepted = row.querySelector('.accepted'); const rejected = row.querySelector('.rejected');
        const syncAccepted = () => { rejected.value = Math.max(0, Number(delivered.value || 0) - Number(accepted.value || 0)); };
        const syncRejected = () => { accepted.value = Math.max(0, Number(delivered.value || 0) - Number(rejected.value || 0)); };
      ...
      const persistReceipt = async () => {
        ...
        const invalidQty = lines.find(line => !Number.isInteger(line.SoLuongGiao) || line.SoLuongGiao < 1 || !Number.isInteger(line.SoLuongChapNhan) || line.SoLuongChapNhan < 0 || !Number.isInteger(line.SoLuongTuChoi) || line.SoLuongTuChoi < 0 || !Number.isFinite(line.DonGiaNhap) || line.DonGiaNhap < 0);
        if (invalidQty) { context.showToast(`Số lượng/đơn giá của ${invalidQty.MaSP} không hợp lệ.`, 'error'); return null; }
  ...
  const initReceiving = async (root, context) => {
    const load = async () => { try { const data = await api(context, '/warehouse/receiving/orders'); root.querySelector('#receivingBody').innerHTML = data.items.length ? data.items.map(item => { const action = item.TrangThaiGiao === 'Đang giao' ? `<button class="warehouse-primary" data-arrive="${esc(item.MaTBGH)}"><svg><use href="#i-truck"></use></svg>Ghi nhận xe đến</button>` : `<button class="warehouse-primary" data-receive="${esc(item.MaTBGH)}"><svg><use href="#i-request"></use></svg>Kiểm nhận hàng</button>`; return `<tr><td><strong>${esc(item.MaTBGH)}</strong><small>Phiếu giao ${esc(item.SoPhieuGiao)}</small></td><td><strong>${esc(item.MaPO)}</strong><small>${esc(item.TenNCC)}</small></td><td><strong>${fmtDate(item.NgayGioDuKienDen)}</strong><small>${esc(item.BienSoXe || 'Chưa có biển số')}${item.SoKien === null || item.SoKien === undefined ? '' : ` · ${item.SoKien} kiện`}</small></td><td class="num"><strong>${item.TongConThieu}</strong><small>đơn vị còn thiếu</small></td><td><span class="status-pill ${statusClass(item.TrangThaiGiao)}">${esc(item.TrangThaiGiao)}</span></td><td><div class="warehouse-row-actions receiving-actions">${action}</div></td></tr>`; }).join('') : '<tr><td colspan="6" class="warehouse-empty"><strong>Chưa có chuyến hàng nào được báo đang giao.</strong><small>Nhân viên mua hàng cần ghi nhận phiếu giao hoặc vận đơn trước khi Thủ kho tiếp nhận.</small></td></tr>'; } catch (error) { context.showToast(error.message, 'error'); } };
    root.innerHTML = `${heading('KHO HÀNG / TIẾP NHẬN', 'Nhận và kiểm tra hàng', 'Theo dõi chuyến giao, ghi nhận xe đến rồi đối chiếu hàng thực tế với Đơn mua.', '<button class="warehouse-secondary" id="openReceipts"><svg><use href="#i-request"></use></svg>Danh sách Phiếu nhập</button>')}<article class="warehouse-hero"><div class="warehouse-hero-copy"><span><svg><use href="#i-truck"></use></svg> KIỂM SOÁT HÀNG ĐẾN KHO</span><h2>Xe đến kho trước, kiểm nhận hàng sau</h2><p>Thủ kho ghi nhận thời điểm xe đến, sau đó mới kiểm số lượng, chất lượng, lô và hạn dùng. Chỉ Phiếu nhập đã xác nhận mới làm tăng tồn kho.</p></div></article><article class="warehouse-table-card"><div class="warehouse-panel-title"><div><p>CHUYẾN GIAO ĐANG THỰC HIỆN</p><h2>Hàng chờ tiếp nhận</h2></div><button class="warehouse-secondary" id="refreshReceiving"><svg><use href="#i-refresh"></use></svg>Làm mới</button></div><div class="warehouse-table-wrap"><table class="warehouse-table receiving-table"><colgroup><col style="width:17%"><col style="width:22%"><col style="width:21%"><col style="width:13%"><col style="width:12%"><col style="width:15%"></colgroup><thead><tr><th>CHUYẾN GIAO</th><th>ĐƠN MUA / NCC</th><th>DỰ KIẾN ĐẾN</th><th>CÒN CHỜ GIAO</th><th>TRẠNG THÁI</th><th>THAO TÁC</th></tr></thead><tbody id="receivingBody"></tbody></table></div></article>`;
    root.querySelector('#openReceipts').addEventListener('click', () => context.navigate('warehouse-receipts')); root.querySelector('#refreshReceiving').addEventListener('click', load); root.addEventListener('click', async event => { const arrive = event.target.closest('[data-arrive]'); if (arrive) { arrive.disabled = true; try { const result = await api(context, `/warehouse/receiving/shipments/${arrive.dataset.arrive}/arrive`, { method: 'POST' }); context.showToast(result.message, 'success'); await load(); } catch (error) { arrive.disabled = false; context.showToast(error.message, 'error'); } return; } const receive = event.target.closest('[data-receive]'); if (receive) receivingModal(context, receive.dataset.receive, load); }); await load();
  };

  const initReceipts = async (root, context) => {
    const load = async () => { try { const search = root.querySelector('#receiptSearch').value; const status = root.querySelector('#receiptStatus').value; const data = await api(context, `/warehouse/receipts?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}`); root.querySelector('#receiptBody').innerHTML = data.items.length ? data.items.map(item => `<tr><td><strong>${esc(item.MaPN)}</strong><small>${fmtDate(item.NgayNhap)}</small></td><td><strong>${esc(item.MaPO)}</strong><small>${esc(item.TenNCC)}</small></td><td>${esc(item.TenKho)}</td><td class="num"><strong>${item.TongChapNhan}</strong><small>đạt / ${item.TongGiao} giao</small></td><td class="num">${item.TongTuChoi}</td><td class="num"><strong>${money(item.TongTien)}</strong></td><td><span class="status-pill ${statusClass(item.TrangThai)}">${esc(item.TrangThai)}</span></td><td><button class="warehouse-secondary" data-view-receipt="${esc(item.MaPN)}">${item.TrangThai === 'Nháp' ? 'Kiểm tra & xác nhận' : 'Chi tiết'}</button></td></tr>`).join('') : `<tr><td colspan="8" class="warehouse-empty">${esc(window.FLY_SEARCH?.emptyMessage?.(search, 'phiếu nhập', 'Chưa có Phiếu nhập phù hợp.') || 'Chưa có Phiếu nhập phù hợp.')}</td></tr>`; } catch (error) { context.showToast(error.message, 'error'); } };
    root.innerHTML = `${heading('KHO HÀNG / PHIẾU NHẬP', 'Phiếu nhập kho', 'Theo dõi các lần giao hàng, phần chấp nhận và phần từ chối theo từng Đơn mua.', '<button class="warehouse-primary" id="newReceiving"><svg><use href="#i-truck"></use></svg>Nhận lô hàng mới</button>')}<article class="warehouse-table-card"><div class="warehouse-toolbar"><label class="warehouse-search"><svg><use href="#i-search"></use></svg><input id="receiptSearch" placeholder="Tìm mã Phiếu nhập, Đơn mua hoặc Nhà cung cấp..." value="${esc(window.FLY_SEARCH?.takePendingQuery?.('warehouse-receipts') || '')}"></label><div class="warehouse-toolbar-actions"><select id="receiptStatus"><option value="">Tất cả trạng thái</option><option>Nháp</option><option>Đã xác nhận</option></select><button class="warehouse-icon-button" id="refreshReceipts"><svg><use href="#i-refresh"></use></svg></button></div></div><div class="warehouse-table-wrap"><table class="warehouse-table"><thead><tr><th>PHIẾU NHẬP</th><th>ĐƠN MUA / NCC</th><th>KHO NHẬN</th><th>CHẤP NHẬN</th><th>TỪ CHỐI</th><th>GIÁ TRỊ NHẬP</th><th>TRẠNG THÁI</th><th>THAO TÁC</th></tr></thead><tbody id="receiptBody"></tbody></table></div></article>`;
