(() => {
  ...
  const heading = (title, subtitle, action) => `<header class="warehouse-heading"><div><p class="warehouse-kicker">${esc(phrase('MUA HÀNG / DỮ LIỆU NHÀ CUNG CẤP'))}</p><h1>${esc(phrase(title))}</h1><p>${esc(phrase(subtitle))}</p></div>${action}</header>`;
  ...
  const initSuppliers = async (root, context) => {
    ...
    const loadExtensionQueue = async () => {
      ...
      try {
        ...
        box.hidden = false;
        box.innerHTML = `<div class="manager-readonly-note"><svg><use href="#i-report"></use></svg><div><strong>Kế toán nhờ xin gia hạn NCC</strong><span>Liên hệ nhà cung cấp, rồi ghi hạn mới. Không cần kênh chat riêng.</span></div></div>
          <div class="warehouse-table-wrap"><table class="warehouse-table"><thead><tr><th>NCC / CÔNG NỢ</th><th>CÒN LẠI</th><th>HẠN CŨ</th><th></th></tr></thead><tbody>${rows.map(row => `<tr><td><strong>${esc(row.TenNCC)}</strong><small>${esc(row.MaCNPTra)} · ${esc(row.NguoiYeuCau || '')}</small></td><td class="num">${money(row.SoTienConLai)}</td><td>${fmtDate(row.HanCu)}</td><td><button type="button" class="warehouse-primary" data-extend-grant="${esc(row.MaCNPTra)}">Ghi hạn mới</button></td></tr>`).join('')}</tbody></table></div>`;
      } catch {
        box.hidden = true;
      }
    };
    root.innerHTML = `${heading('Nhà cung cấp', 'Quản lý hồ sơ, thông tin liên hệ và trạng thái hợp tác trước khi lập Đơn mua.', '<button class="warehouse-primary" id="newSupplier"><svg><use href="#i-plus"/></svg>Thêm Nhà cung cấp</button>')}
      <div id="supplierExtensionQueue" class="supplier-extension-queue" hidden></div>
    ...
    root.addEventListener('click', async event => {
      ...
      if (grant) {
        if (window.FLY_PAYABLES?.grantExtension) await window.FLY_PAYABLES.grantExtension(context, grant.dataset.extendGrant, load);
        return;
      }
