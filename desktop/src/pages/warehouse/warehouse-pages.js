  ...
  const initWarehouseReturns = async (root, context) => {
    const inspect = async (id, onDone) => {
      try {
        ...
        overlay.innerHTML = `<div class="warehouse-modal receipt-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">KIỂM TRA HÀNG KHÁCH TRẢ / UC21</p><h2>${esc(detail.ticket.MaDT)}</h2></div><button class="warehouse-icon-button close">×</button></div><div class="warehouse-modal-body"><div class="return-source-card"><div><span>HÓA ĐƠN GỐC</span><strong>${esc(detail.ticket.MaHD)}</strong><small>${fmtDate(detail.ticket.NgayHoaDon)}</small></div><div><span>CA / THU NGÂN GỐC</span><strong>${esc(detail.ticket.MaCaGoc || '—')}</strong><small>${esc(detail.ticket.ThuNganGoc || '')}</small></div><div><span>HÌNH THỨC</span><strong>${esc(detail.ticket.HinhThucXuLy)}</strong></div><div><span>KHÁCH</span><strong>${esc(detail.ticket.TenKH || 'Khách vãng lai')}</strong><small>${esc(detail.ticket.SDT || '')}</small></div></div><div class="return-inspect-warn"><svg><use href="#i-warning"/></svg><div><strong>Hàng hỏng, hết hạn hoặc lỗi cửa hàng: bỏ tick nhập lại kho</strong><span>Chỉ tick nhập lại khi bao bì nguyên, còn hạn và còn bán được. Hàng không nhập lại không hoàn giá vốn vào tồn, ảnh hưởng lãi gộp theo công thức Kế toán.</span></div></div><p>Lý do thu ngân: ${esc(detail.ticket.LyDo || '—')}</p><div class="warehouse-table-wrap"><table class="warehouse-table"><thead><tr><th>SẢN PHẨM</th><th>SL</th><th>THÀNH TIỀN</th></tr></thead><tbody>${detail.lines.filter(item => item.LoaiDong === 'Hàng khách trả').map(item => `<tr><td><strong>${esc(item.TenSP)}</strong><small>${esc(item.MaSP)}</small></td><td class="num">${item.SoLuong}</td><td class="num">${Number(item.ThanhTien).toLocaleString('vi-VN')}</td></tr>`).join('')}</tbody></table></div><label class="warehouse-field return-restock-field"><input type="checkbox" id="restock"> Hàng đạt yêu cầu, được nhập lại kho</label><div class="warehouse-field"><label>Kết quả kiểm tra *</label><textarea id="inspectNote" maxlength="200" placeholder="Tình trạng bao bì, hạn dùng, lỗi sản phẩm..."></textarea></div></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close">Hủy</button><button class="warehouse-primary save-inspect">Ghi nhận và gửi duyệt</button></div></div>`;
        document.body.appendChild(overlay);
