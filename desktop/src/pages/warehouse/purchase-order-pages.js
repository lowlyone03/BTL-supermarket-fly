(() => {
  ...
  const orderDetailModal = async (context, id, manager = false, onDone = async () => {}) => {
    try {
      ...
      document.body.appendChild(overlay); const close = () => overlay.remove(); overlay.querySelectorAll('.close').forEach(button => button.addEventListener('click', close));
      overlay.querySelector('.print-order').addEventListener('click', () => printOrder(data));
  ...
  const createOrderModal = async (context, onDone) => {
    try {
      ...
      overlay.querySelector('.save-order').addEventListener('click', () => save(false)); overlay.querySelector('.print-order-draft').addEventListener('click', () => save(false, true)); overlay.querySelector('.submit-order').addEventListener('click', () => save(true));
      await loadLines();
    } catch (error) { context.showToast(error.message, 'error'); }
  };
    ...
    try {
      ...
      const save = async (submit, preview = false) => {
        ...
        try {
          ...
          context.showToast(preview ? 'Đã lưu. Mở xem trước để in.' : (submit ? 'Đã cập nhật và gửi lại Quản lý phê duyệt.' : result.message), 'success'); close(); await onDone();
          if (preview) printOrder(await api(context, `/purchasing/purchase-orders/${id}`));
        } catch (error) { context.showToast(error.message, 'error'); }
      };
      overlay.querySelector('.save-edit').addEventListener('click', () => save(false));
      overlay.querySelector('.print-order-edit')?.addEventListener('click', () => save(false, true));
      overlay.querySelector('.resubmit-edit').addEventListener('click', () => save(true));
  ...
  const initOrders = async (root, context) => {
    const load = async () => {
      try {
        ...
        }).join('') : `<tr><td colspan="6" class="warehouse-empty">${esc(window.FLY_SEARCH?.emptyMessage?.(search, 'đơn mua', 'Chưa có Đơn mua hàng phù hợp.') || 'Chưa có Đơn mua hàng phù hợp.')}</td></tr>`;
      } catch (error) { context.showToast(error.message, 'error'); }
    };
  ...
  const inventoryCountApprovalModal = async (context, id, onDone) => {
    try {
      ...
      const blockingStock = stockWarnings.filter(item => !item.alreadyReduced);
      const defaultRejectReason = data.suggestedRejectReason
        || (blockingStock.length
          ? `Không duyệt trên số liệu cũ — Thủ kho đếm lại. ${blockingStock.map(item => item.message).join(' ')}`.slice(0, 500)
          : '');
      const warningHtml = stockWarnings.length
        ? `<div class="manager-readonly-note"><svg><use href="#i-warning"/></svg><div><strong>${blockingStock.length ? 'Tồn dòng chênh lệch đã đổi sau lúc đếm — bấm Từ chối để Thủ kho đếm lại' : 'Tồn dòng chênh lệch đã được cập nhật trước'}</strong>${stockWarnings.map(item => `<span>${esc(item.message)}</span>`).join('')}</div></div>`
        : '';
      const overlay = document.createElement('div');
      overlay.className = 'warehouse-modal-backdrop';
      overlay.innerHTML = `<div class="warehouse-modal inventory-count-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">PHÊ DUYỆT ĐIỀU CHỈNH TỒN / ${esc(count.MaKK)}</p><h2>${esc(count.TenKho)}</h2><span>${fmtDate(count.NgayKiemKe)} · Thủ kho ${esc(count.NguoiKiemKe)}</span></div><button class="warehouse-icon-button close" type="button">×</button></div><div class="warehouse-modal-body"><div class="manager-readonly-note"><svg><use href="#i-warning"/></svg><div><strong>Phê duyệt sẽ cập nhật tồn kho</strong><span>Hệ thống chỉ sửa tồn cho các dòng chênh lệch và đồng thời tạo Giao dịch kho loại Điều chỉnh. Từ chối sẽ giữ nguyên tồn.</span></div></div>${warningHtml}<div class="warehouse-stats"><article><span>TỔNG MẶT HÀNG</span><strong>${data.lines.length}</strong><small>Trong đợt kiểm kê</small></article><article class="attention"><span>CÓ CHÊNH LỆCH</span><strong>${differences.length}</strong><small>Cần xem nguyên nhân</small></article><article><span>TỔNG THỪA</span><strong>${differences.reduce((sum, line) => sum + Math.max(0, Number(line.ChenhLech)), 0)}</strong><small>Đơn vị hàng</small></article><article><span>TỔNG THIẾU</span><strong>${differences.reduce((sum, line) => sum + Math.max(0, -Number(line.ChenhLech)), 0)}</strong><small>Đơn vị hàng</small></article></div><div class="warehouse-table-wrap"><table class="warehouse-table inventory-count-table"><thead><tr><th>SẢN PHẨM</th><th>HỆ THỐNG</th><th>THỰC TẾ</th><th>CHÊNH LỆCH</th><th>KẾT QUẢ</th><th>TÌNH TRẠNG</th><th>NGUYÊN NHÂN</th></tr></thead><tbody>${data.lines.map(line => `<tr><td><strong>${esc(line.TenSP)}</strong><small>${esc(line.MaSP)} · ${esc(line.DonViTinh)}</small></td><td class="num">${line.SLHeThong}</td><td class="num"><strong>${line.SLThucTe}</strong></td><td class="num"><strong>${Number(line.ChenhLech) > 0 ? '+' : ''}${line.ChenhLech}</strong></td><td><span class="status-pill ${Number(line.ChenhLech) === 0 ? 'ok' : 'sent'}">${esc(line.KetQuaDoiChieu)}</span></td><td>${esc(line.TinhTrangHang || 'Bình thường')}</td><td>${esc(line.NguyenNhan || '—')}</td></tr>`).join('')}</tbody></table></div>${count.GhiChu ? `<div class="receipt-rule"><svg><use href="#i-report"/></svg><span>${esc(count.GhiChu)}</span></div>` : ''}</div><div class="count-reject-panel${blockingStock.length ? ' is-open' : ''}"><div class="warehouse-field"><label>Lý do từ chối — Thủ kho sẽ nhận thông báo đếm lại *</label><textarea id="countRejectReason" maxlength="500" rows="4" placeholder="Ví dụ: Tồn đã đổi vì nhập hàng PN...">${esc(defaultRejectReason)}</textarea></div><p class="warehouse-modal-note">Tồn giữ nguyên. Thủ kho tạo đợt kiểm kê mới trên số tồn hiện tại (sau phiếu nhập / xuất).</p><div class="warehouse-modal-actions" style="padding:12px 0 0;border:0;background:transparent"><button class="warehouse-danger confirm-reject-count" type="button">Gửi Thủ kho đếm lại</button></div></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close" type="button">Đóng</button><button class="warehouse-danger reject-count${blockingStock.length ? ' is-required' : ''}" type="button">Từ chối và báo Thủ kho đếm lại</button><button class="warehouse-primary approve-count" type="button"${blockingStock.length ? ' disabled title="Không duyệt được vì tồn dòng chênh lệch đã đổi"' : ''}>Duyệt và điều chỉnh tồn</button></div></div>`;
      document.body.appendChild(overlay);
      const close = () => overlay.remove();
      overlay.querySelectorAll('.close').forEach(button => button.addEventListener('click', close));
      ...
      const submitReject = async () => {
        const reason = (overlay.querySelector('#countRejectReason')?.value || '').trim();
        if (!reason) return context.showToast('Vui lòng nhập lý do từ chối để Thủ kho đếm lại.', 'error');
        try {
          const result = await api(context, `/admin/approvals/inventory-counts/${id}/reject`, { method: 'POST', body: JSON.stringify({ LyDo: reason }) });
          context.showToast(result.message, 'success'); close(); await onDone();
        } catch (error) { context.showToast(error.message, 'error'); }
      };
      overlay.querySelector('.reject-count').addEventListener('click', () => {
        const panel = overlay.querySelector('.count-reject-panel');
        const field = overlay.querySelector('#countRejectReason');
      ...
      overlay.querySelector('.confirm-reject-count')?.addEventListener('click', submitReject);
    } catch (error) { context.showToast(error.message, 'error'); }
  };
  ...
  const paymentVoucherApprovalModal = async (context, id, onDone) => {
    try {
      ...
      const overlay = document.createElement('div');
      overlay.className = 'warehouse-modal-backdrop';
      const fundDefault = voucher.PhuongThuc === 'Tiền mặt' ? 'Tiền mặt' : 'Ủy quyền chuyển khoản';
      overlay.innerHTML = `<div class="warehouse-modal payment-voucher-approval-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">DUYỆT VÀ GIAO TIỀN CHO KẾ TOÁN</p><h2>${esc(voucher.MaPhieu)}</h2><span>${esc(voucher.TenNCC)} · ${money(voucher.SoTienPhieuChi)}</span></div><button class="warehouse-icon-button close" type="button">×</button></div><div class="warehouse-modal-body"><div class="manager-readonly-note"><svg><use href="#i-cash"></use></svg><div><strong>Duyệt Phiếu chi là bước giao quỹ</strong><span>Bạn giao đúng ${money(voucher.SoTienPhieuChi)} trên phiếu này (có thể là từng phần công nợ) cho Kế toán. Công nợ còn lại ${money(voucher.SoTienConLai)} chỉ giảm sau khi Kế toán thanh toán thành công. Trợ lý AI không được duyệt hộ.</span></div></div><div class="payment-voucher-source"><div><span>CÔNG NỢ</span><strong>${esc(voucher.MaCNPTra)}</strong></div><div><span>HẠN THANH TOÁN</span><strong>${fmtDate(voucher.HanThanhToan)}</strong></div><div><span>HÓA ĐƠN NCC</span><strong>${esc(voucher.SoHoaDon)}</strong></div><div><span>ĐỐI CHIẾU BA BÊN</span><strong><i class="status-pill ${voucher.TrangThaiDoiChieu === 'Đã khớp' ? 'ok' : 'cancelled'}">${esc(voucher.TrangThaiDoiChieu)}</i></strong></div><div><span>ĐƠN MUA</span><strong>${esc(voucher.MaPO)}</strong></div><div><span>PHIẾU NHẬP</span><strong>${esc(voucher.MaPN)}</strong></div><div><span>CÁCH CHI NCC</span><strong>${esc(voucher.PhuongThuc)}</strong></div><div><span>KẾ TOÁN LẬP</span><strong>${esc(voucher.NguoiLap)}</strong></div></div><div class="payment-voucher-amount"><span>SỐ TIỀN PHẢI GIAO / CÔNG NỢ CÒN LẠI</span><strong>${money(voucher.SoTienPhieuChi)} / ${money(voucher.SoTienConLai)}</strong><small>Hai số tiền bắt buộc bằng nhau. Giao đủ một lần, không ứng trước.</small></div><div class="warehouse-form-grid payment-voucher-fields"><div class="warehouse-field"><label>Cách giao tiền cho Kế toán *</label><select id="fundMethod"><option value="Tiền mặt" ${fundDefault === 'Tiền mặt' ? 'selected' : ''}>Tiền mặt — giao đủ cho Kế toán</option><option value="Ủy quyền chuyển khoản" ${fundDefault === 'Ủy quyền chuyển khoản' ? 'selected' : ''}>Ủy quyền chuyển khoản — Kế toán dùng TK cửa hàng</option></select></div><div class="warehouse-field full"><label>Ghi chú giao quỹ</label><textarea id="fundNote" maxlength="500" rows="2" placeholder="Ví dụ: đã giao đủ tiền mặt tại văn phòng / ủy quyền chi từ tài khoản Vietcombank cửa hàng"></textarea></div><div class="warehouse-field full reject-reason-field" hidden><label>Lý do từ chối *</label><textarea id="rejectFundReason" maxlength="500" rows="2" placeholder="Bắt buộc khi từ chối Phiếu chi"></textarea></div></div><div class="manager-readonly-note"><svg><use href="#i-request"></use></svg><div><strong>Nội dung chi</strong><span>${esc(voucher.NoiDung)}</span>${voucher.GhiChu ? `<small>${esc(voucher.GhiChu)}</small>` : ''}</div></div><div class="warehouse-table-wrap"><table class="warehouse-table"><thead><tr><th>MẶT HÀNG HÓA ĐƠN</th><th>SỐ LƯỢNG</th><th>ĐƠN GIÁ</th><th>THUẾ</th><th>THÀNH TIỀN</th></tr></thead><tbody>${data.lines.map(line => `<tr><td><strong>${esc(line.TenSP)}</strong><small>${esc(line.MaSP)} · ${esc(line.DonViTinh)}</small></td><td class="num">${line.SoLuong}</td><td class="num">${money(line.DonGia)}</td><td class="num">${line.ThueSuat}%</td><td class="num"><strong>${money(line.ThanhTien)}</strong></td></tr>`).join('')}</tbody></table></div></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close" type="button">Đóng</button><button class="warehouse-danger reject-payment-voucher" type="button">Từ chối</button><button class="warehouse-primary approve-payment-voucher" type="button">Duyệt và giao tiền</button></div></div>`;
      document.body.appendChild(overlay);
      const close = () => overlay.remove();
      overlay.querySelectorAll('.close').forEach(button => button.addEventListener('click', close));
      overlay.querySelector('.approve-payment-voucher').addEventListener('click', async event => {
  ...
  const payrollVoucherApprovalModal = async (context, id, onDone) => {
    try {
      const data = await api(context, `/admin/approvals/payroll-vouchers/${id}`);
      const voucher = data.voucher;
      const overlay = document.createElement('div');
      overlay.className = 'warehouse-modal-backdrop';
      overlay.innerHTML = `<div class="warehouse-modal payment-voucher-approval-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">DUYỆT PHIẾU CHI LƯƠNG</p><h2>${esc(voucher.MaPhieu)}</h2><span>${esc(voucher.TenNV || '')} · kỳ ${esc(voucher.MaKy)} · ${money(voucher.SoTien)}</span></div><button class="warehouse-icon-button close" type="button">×</button></div><div class="warehouse-modal-body"><div class="manager-readonly-note"><svg><use href="#i-cash"></use></svg><div><strong>Duyệt là đồng ý chi cho người này — chưa đưa tiền</strong><span>Sau khi duyệt xong kỳ, bạn giao quỹ cho kế toán một lần. Kế toán mới chi từng nhân viên. Phiếu chi Nhà cung cấp vẫn duyệt + giao theo từng phiếu như cũ.</span></div></div><div class="payment-voucher-source"><div><span>NHÂN VIÊN</span><strong>${esc(voucher.TenNV || voucher.MaNV)}</strong></div><div><span>KỲ LƯƠNG</span><strong>${esc(voucher.MaKy)}</strong></div><div><span>TẤT TOÁN DỰ KIẾN</span><strong>${fmtDate(data.period?.NgayTraDuKien)}</strong></div><div><span>KÊNH CHI</span><strong>${esc(voucher.PhuongThuc)}</strong></div><div><span>SỐ TIỀN KHÓA</span><strong>${money(voucher.SoTien)}</strong></div></div><div class="warehouse-form-grid payment-voucher-fields"><div class="warehouse-field full reject-reason-field" hidden><label>Lý do từ chối *</label><textarea id="payrollRejectReason" maxlength="500" rows="2"></textarea></div></div></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close" type="button">Đóng</button><button class="warehouse-danger reject-payroll-voucher" type="button">Từ chối</button><button class="warehouse-primary approve-payroll-voucher" type="button">Duyệt</button></div></div>`;
      document.body.appendChild(overlay);
      const close = () => overlay.remove();
      overlay.querySelectorAll('.close').forEach(button => button.addEventListener('click', close));
  ...
  const payrollHandoverModal = async (context, maKy, fund, onDone) => {
    const overlay = document.createElement('div');
    overlay.className = 'warehouse-modal-backdrop';
    const tm = Number(fund?.tmTopUp || 0);
    const ck = Number(fund?.ckTopUp || 0);
    const receiver = accountantLabel(fund);
    overlay.innerHTML = `<div class="warehouse-modal payment-voucher-approval-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">GIAO QUỸ CHO KẾ TOÁN</p><h2>Kỳ ${esc(maKy)}</h2><span>Người nhận: ${esc(receiver)}</span></div><button class="warehouse-icon-button close" type="button">×</button></div><div class="warehouse-modal-body"><div class="manager-readonly-note"><svg><use href="#i-cash"></use></svg><div><strong>Bạn đưa một cục tiền / quyền cho kế toán — không đưa từng nhân viên</strong><span>Tiền mặt: giao đủ tổng các phiếu TM đã duyệt chưa chi. Chuyển khoản: ủy quyền tài khoản cửa hàng đúng tổng CK. ${esc(fund?.accountant?.TenNV || 'Kế toán')} sẽ chi từng người từ quỹ này.</span></div></div><div class="payment-voucher-amount"><span>CẦN GIAO CHO KẾ TOÁN BÂY GIỜ</span><strong>Tiền mặt ${money(tm)} · Ủy quyền CK ${money(ck)}</strong><small>Nếu đã giao trước đó, chỉ bổ sung phần phiếu mới duyệt. Không giao lần hai cùng kênh khi đã đủ.</small></div><div class="warehouse-field full"><label>Ghi chú</label><textarea id="payrollHandoverNote" maxlength="500" rows="2" placeholder="Ví dụ: đã giao đủ tiền mặt tại văn phòng / ủy quyền TK Vietcombank cửa hàng"></textarea></div></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close" type="button">Hủy</button><button class="warehouse-primary confirm-handover" type="button">Xác nhận giao quỹ cho kế toán</button></div></div>`;
    document.body.appendChild(overlay);
    const close = () => overlay.remove();
    overlay.querySelectorAll('.close').forEach(button => button.addEventListener('click', close));
  ...
  const payrollHandoverHistoryModal = (period) => {
    const snap = period.fund || {};
    const fund = snap.fund;
    const overlay = document.createElement('div');
    overlay.className = 'warehouse-modal-backdrop';
    overlay.innerHTML = `<div class="warehouse-modal payment-voucher-approval-modal"><div class="warehouse-modal-heading"><div><p class="warehouse-kicker">LỊCH SỬ GIAO QUỸ</p><h2>Kỳ ${esc(period.MaKy)}</h2><span>Đã giao cho ${esc(accountantLabel(snap))}</span></div><button class="warehouse-icon-button close" type="button">×</button></div><div class="warehouse-modal-body"><div class="payment-voucher-source"><div><span>NGƯỜI GIAO</span><strong>${esc(fund?.TenQL || 'Quản lý')}</strong></div><div><span>NGÀY GIAO</span><strong>${fmtDateTime(fund?.NgayGiao)}</strong></div><div><span>NGƯỜI NHẬN</span><strong>${esc(accountantLabel(snap))}</strong></div><div><span>TIỀN MẶT ĐÃ GIAO / CÒN</span><strong>${money(fund?.SoTienMatGiao)} / ${money(fund?.SoTienMatCon)}</strong></div><div><span>CK ĐÃ ỦY QUYỀN / CÒN</span><strong>${money(fund?.SoTienCKGiao)} / ${money(fund?.SoTienCKCon)}</strong></div></div>${fund?.GhiChu ? `<p>${esc(fund.GhiChu)}</p>` : ''}<p class="warehouse-modal-note">Không giao lần hai cùng kênh khi quỹ đã đủ cho các phiếu đã duyệt. Chỉ bổ sung nếu còn phiếu mới duyệt.</p></div><div class="warehouse-modal-actions"><button class="warehouse-secondary close" type="button">Đóng</button></div></div>`;
    document.body.appendChild(overlay);
  ...
  const initApprovalCenter = async (root, context) => {
    ...
    root.addEventListener('click', async event => {
      ...
      const reviewPayrollVoucher = event.target.closest('[data-review-payroll-voucher]');
      ...
      const handoverHistory = event.target.closest('[data-handover-history]');
      if (handoverHistory) {
        try {
          const board = await api(context, '/admin/approvals/payroll-board');
          const period = (board.periods || []).find(item => item.MaKy === handoverHistory.dataset.handoverHistory);
          if (period) payrollHandoverHistoryModal(period);
        } catch (error) { context.showToast(error.message, 'error'); }
        return;
      }
      const handoverPayroll = event.target.closest('[data-handover-payroll]');
      if (handoverPayroll) {
        const ky = handoverPayroll.dataset.handoverPayroll;
        let fund = {};
        try {
          const board = await api(context, '/admin/approvals/payroll-board');
          fund = (board.periods || []).find(item => item.MaKy === ky)?.fund || {};
        } catch { /* dùng số 0 */ }
        return payrollHandoverModal(context, ky, fund, load);
      }
  ...
  window.FLY_PAYMENT_VOUCHER = { openApproval: paymentVoucherApprovalModal };
