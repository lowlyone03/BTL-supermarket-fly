(() => {
  ...
  const officialCss = `
      ...
      .vn-state{text-align:center}
      .vn-state em{display:block;margin-top:3px;font-size:13px;font-style:italic;font-weight:700}
      .vn-state i{display:block;width:42%;margin:7px auto 0;border-top:1px solid #000}
      .vn-title{margin:18px 0 4px;text-align:center;font-size:16px;font-weight:700;text-transform:uppercase}
      .vn-sub{margin:0;text-align:center;font-size:13px}
      .vn-place{margin:10px 0 14px;text-align:right;font-style:italic}
      .vn-fields{width:100%;margin:0 0 12px;border-collapse:collapse}
      .vn-fields td{padding:3px 6px 3px 0;border:0;vertical-align:top}
      .vn-fields td:nth-child(odd){width:22%;font-weight:700}
      table.doc-table{width:100%;border-collapse:collapse;page-break-inside:auto}
      table.doc-table th,table.doc-table td{border:1px solid #000;padding:6px 7px;vertical-align:top}
      table.doc-table th{text-align:center;font-size:12px;background:#fff}
      tbody tr{page-break-inside:avoid}
      .center{text-align:center}.right{text-align:right}.row-index{width:28px}
      .vn-totals{margin:12px 0 0 auto;width:min(95mm,100%)}
      .vn-totals div{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px dotted #000}
      .vn-note{margin-top:12px;line-height:1.45}
      .vn-sign{display:grid;grid-template-columns:repeat(var(--signatures),1fr);gap:18px;margin-top:28px;text-align:center;page-break-inside:avoid}
      .vn-sign strong,.vn-sign span{display:block}
      .vn-sign span{margin-top:4px;font-size:12px;font-style:italic}
      .vn-sign .space{height:56px}
      .vn-foot{margin-top:18px;padding-top:8px;border-top:1px solid #000;font-size:11px}
      @media print{body{background:#fff}.sheet{width:auto;min-height:0;margin:0;padding:0}}
    `;

  const pageBox = orientation => orientation === 'landscape'
    ? { w: '297mm', h: '210mm' }
    : { w: '210mm', h: '297mm' };

  const fillCss = (css, orientation) => {
  ...
  const systemMarkup = (config, parts) => {
    ...
    const summaries = parts.summaryItems.map((item, index) => `<article style="--summary-color:${item.color || summaryColors[index % 4]}"><span>${esc(item.label)}</span><strong>${esc(formatted(item))}</strong>${item.hint ? `<small>${esc(item.hint)}</small>` : ''}</article>`).join('');
  ...
  const officialMarkup = (config, parts) => {
    const issued = vnParts(config.documentDate);
    const fields = parts.fields.map(item => `<tr><td>${esc(item.label)}</td><td>${esc(item.text)}</td></tr>`).join('');
    const summaryRows = parts.summaryItems.map(item => `<tr><td>${esc(item.label)}</td><td class="right">${esc(formatted(item))}</td></tr>`).join('');
    const totals = parts.totalsConfig.map(item => `<div><span>${esc(item.label)}</span><strong>${esc(formatted(item))}</strong></div>`).join('');
    ...
    return `<main class="sheet">
      ...
      <h1 class="vn-title">${esc(config.title)}</h1>
      <p class="vn-sub">Số: ${esc(config.number || '—')}${config.status ? ` · ${esc(config.status)}` : ''}</p>
      <p class="vn-place">Hà Nội, ngày ${esc(issued.day)} tháng ${esc(issued.month)} năm ${esc(issued.year)}</p>
      ...
      <footer class="vn-foot">In từ hệ thống quản lý nội bộ SuperMarket FLY. Bản giấy trắng mực đen dùng nộp lưu / ký xác nhận.</footer>
    </main>`;
  };
  ...
  const show = config => {
    ...
    if (old) old.remove();
    const isReport = isReportDoc(config);
    let skin = config.skin === 'official' ? 'official' : 'system';
    ...
    const render = () => {
      ...
      kicker.textContent = skin === 'official'
        ? 'BẢN GIẤY TRẮNG MỰC ĐEN · CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM'
        : (isReport ? 'BẢN HỆ THỐNG · BÁO CÁO QUẢN TRỊ' : 'BẢN HỆ THỐNG · CHỨNG TỪ MÀU');
      iframe.srcdoc = build({ ...config, skin });
    };
    iframe.addEventListener('load', () => {
      if (skin === 'official' || !isReport || !iframe.contentDocument?.head) return;
    ...
    overlay.querySelectorAll('.print-skin').forEach(button => button.addEventListener('click', () => {
